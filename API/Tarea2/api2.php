...
_END;
    ...
    case 'DELETE':
        ...
        if (isset($_GET['id']) && !is_null($_GET['id'])) {
            $jugador = new clubbasket();
            $arrayJson = array('id' => $_GET['id']);
            $datos_encode = json_encode($arrayJson);
            $resultado = $jugador->borrarJugador($datos_encode);

            if ($resultado != null) {
                echo "<p>Se ha borrado correctamente el jugador con id: <b>" . $_GET['id'] . "</b></p>";
            } else {
                echo "<p>Hubo un error en el borrado del jugador.</p>";
            }
        } else {
            echo "<p>Error, no se han especificado los parámetros</p>";
        }
