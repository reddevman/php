...
<body>
    ...
    $numero = substr($_POST['numeroEmpleado'],1, -1);
    $nombre = $_POST['nombre'];
    ...
    if (
        isset($numero, $nombre, $puesto, $jefe, $fechaAlta, $salario, $comision, $numDpto) &&
        !empty($numero) && !empty($nombre) && !empty($puesto) && !empty($jefe) && !empty($fechaAlta) &&
        !empty($salario) && !empty($comision) && !empty($numDpto)
    ) {
        $bbdd = new Empresa();
        $bbdd->modificarEmpleado($numero, $nombre, $puesto, $jefe, $fechaAlta, $salario, $comision, $numDpto);
    } else {
        echo "No se puede hacer la inserción de los datos del empleado";
    }
    ?>

    <h3>El último registro actualizado es:</h3>

    <?php
        echo "<b>Número Empleado: </b>" . $numero . "<br>";
